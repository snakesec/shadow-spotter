...
func AllowUnsafe(v bool) ScanOption {
...
func UserQuery(query string) ScanOption {
	return func(o *ScanOptions) error {
		o.UserQuery = query
		return nil
	}
...
func LoadTextWordlist(fns []string, extensions []string, dirsearchCompatabilityMode bool, httpMethod string, customQuery string) ScanOption {
	return func(o *ScanOptions) error {
		...
		for _, filename := range fns {
			...
			for _, v := range lines {
				...
				methodresult, _ := http.MethodFromString(httpMethod)

				o.Routes = append(o.Routes, &http.Route{Method: methodresult, Path: v, Query: []byte(customQuery)})
				...
				for _, ext := range extensions {
					...
					o.Routes = append(o.Routes, &http.Route{Method: methodresult, Path: path})
				}
			}
		}
		return nil
...
func LoadAssetnoteWordlist(fns []string, extensions []string, dirsearchCompatabilityMode bool, httpMethod string, customQuery string) ScanOption {
	return func(o *ScanOptions) error {
		o.extensions = append(o.extensions, extensions...)
		...
		for _, v := range wms {
			...
			for _, vv := range w {
				...
				o.Routes = append(o.Routes, &http.Route{Method: methodresult, Path: []byte( vv ), Query: []byte(customQuery)})
...
func LoadAssetnoteWordlistKitebuilder(fns []string) ScanOption {
	return func(o *ScanOptions) error {
		...
		for _, v := range wms {
			...
			routes, err := proute.APIsToKiterunnerRoutes(api, o.AllowUnsafe, o.ForceMethod, o.UserQuery)
...
func LoadKitebuilderFile(fns []string) ScanOption {
	return func(o *ScanOptions) error {
		for _, filename := range fns {
			...
			for _, v := range apis {
				wcr, err := proute.ToKiterunnerRoutes(v, o.AllowUnsafe, o.ForceMethod, o.UserQuery)
				if errors.As(err, &merr) {
